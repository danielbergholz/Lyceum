...
#     E.g.: docker.io/hexpm/elixir:1.20.0-erlang-28.3-debian-trixie-20260610-slim
...
RUN apt-get update \
  && apt-get install -y --no-install-recommends libstdc++6 openssl libncurses6 locales ca-certificates postgresql-client \
  && rm -rf /var/lib/apt/lists/*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/lyceum ./
